...
class Bunch(dict):
    ...
    def tech_corpora(self, filename):

        module_path = dirname(__file__)
        with open(join(module_path, 'Technical_Corpora', filename)) as txt_file:
            text = []
            book_text = txt_file.readlines()

        return Bunch(text = book_text)
